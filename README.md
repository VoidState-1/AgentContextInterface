...
ACI提供一种更直观、更灵活的 AI 交互方式，将Agent的能力推向更广阔的领域。
...
Skills为我们带来的巨大的启发，渐进式的披露是非常优秀的设计。
ACI同样学习到了这点，在ACI中，**操作与窗口绑定**，只有在窗口出现时才会告知AI工具的用法。
这允许我们撰写十分详细的操作指南而不必担心token爆炸。
...
另一方面，ACI可以创建真正的AI原生应用。
通过声明式的框架拼接字符串，从设计之初便完美符合AI的使用习惯。
